...
class RasaClient:
    def __init__(self,
                rasa_url: str = "http://localhost:5009",
                original_number: str = "01568725958",
                rasa_port: Optional[int] = None,
                action_port: Optional[int] = None): # Keep this for future flexibility
    ...
    async def get_next_action(self) -> str:
        """Get the next predicted action from Rasa."""
        try:
            async with self.session.post(
                f"{self.rasa_url}/conversations/{self.sender_id}/predict"
            ) as response:
                prediction = await response.json()
                return prediction.get("scores", [{}])[0].get("action")
        ...
        try:
            with open(output_file, 'w', encoding='utf-8') as outfile:
                ...
                for user_id, user_data in users.items():
                    ...
                    client = RasaClient(original_number=original_number, rasa_port=rasa_port, action_port=action_port)
                    ...
                    for msg_idx, message in enumerate(messages, 1):
                        ...
                        # Print tracker state after bot response
